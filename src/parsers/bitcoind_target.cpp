...
{
    // The path is taken as sent, since bitcoind does not unescape it.
    ...
    // forms, so it is parsed here rather than discarded. An unparsable
    // target simply carries no parameters, as the path remains parsable.
    wallet::uri uri{};
    const auto query = uri.decode(std::string{ path }) ?
        uri.decode_query() : wallet::uri::query_map{};
