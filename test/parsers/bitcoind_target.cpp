...
BOOST_AUTO_TEST_CASE(parsers__bitcoind_target__error_paths__expected)
{
    const std::vector<std::pair<std::string, code>> cases
    {
        ...
        // An escaped separator is not a separator, as bitcoind does not
        // unescape the path.
        { "/rest%2Fblock/" + test_hash + ".bin", server::error::invalid_target },
        { "/rest/block/" + test_hash + "%2F.bin", server::error::invalid_hash }
    };
